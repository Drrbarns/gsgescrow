      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1">Welcome back, {profile?.full_name || 'Seller'}. Here's how your business is doing.</p>
          </div>
          <div className="flex gap-3">
            <WhatsAppShare role="seller" />
            <Link href="/seller/step-1">
              <Button className="rounded-full gap-2 shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all font-bold h-11 px-6">
        ...
        {trust && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <Card className="rounded-3xl border-slate-200 bg-white shadow-xl shadow-slate-200/40 overflow-hidden relative">
              ...
              <CardContent className="flex flex-col gap-8 py-8 sm:flex-row sm:items-center relative z-10">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-4 flex flex-wrap gap-4 text-sm font-medium text-slate-700">
                    <span className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100"><Package className="h-4 w-4 text-blue-500" /> {trust.completed_ok} completed</span>
                    <span className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100"><Star className="h-4 w-4 text-yellow-500" /> {trust.avg_seller_rating?.toFixed(1) || 'N/A'} avg rating</span>
                    <span className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100"><Clock className="h-4 w-4 text-purple-500" /> {trust.avg_delivery_hours ? `${trust.avg_delivery_hours.toFixed(0)}h avg delivery` : 'No data'}</span>
                  </div>
                </div>
                <Link href="/seller/verify">
                  <Button variant="outline" className="rounded-full gap-2 shrink-0 h-11 px-6 font-bold border-slate-200 hover:bg-slate-50">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
          {kpis.map((kpi, i) => (
            <motion.div key={kpi.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
              <Card className="rounded-3xl border-slate-200 shadow-lg shadow-slate-200/30 hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 bg-white">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm font-bold text-slate-500 uppercase tracking-wider">{kpi.label}</span>
                  ...
                  {kpi.sub && <p className="text-sm font-medium text-slate-500 mt-2">{kpi.sub}</p>}
                </CardContent>
              </Card>
            </motion.div>
          ))}
        ...
        <div className="grid gap-6 lg:grid-cols-2 mb-8">
          {/* Revenue Chart */}
          <Card className="rounded-3xl border-slate-200 shadow-lg shadow-slate-200/30 bg-white">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2 text-lg font-bold text-slate-900"><BarChart3 className="h-5 w-5 text-primary" /> Monthly Revenue</CardTitle>
            </CardHeader>
            <CardContent>
              {monthlyData.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-48 text-center bg-slate-50 rounded-2xl border border-slate-100">
          ...
          <Card className="rounded-3xl border-slate-200 shadow-lg shadow-slate-200/30 bg-white">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2 text-lg font-bold text-slate-900"><Award className="h-5 w-5 text-primary" /> Performance</CardTitle>
            </CardHeader>
              ...
              {data?.avg_delivery_hours && (
                <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-black text-slate-900">{data.avg_delivery_hours.toFixed(1)}h</p>
                  </div>
                </div>
              )}
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-3">
                {data.recent_transactions.map((txn: any) => (
                  <div key={txn.id} className="flex items-center justify-between rounded-2xl bg-slate-50 border border-slate-100 px-5 py-4 hover:bg-slate-100 transition-colors cursor-pointer">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-slate-900">{txn.product_name || 'Transaction'}</p>
                        <p className="text-xs font-medium text-slate-500">{format(new Date(txn.created_at), 'dd MMM yyyy')}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-base font-black text-slate-900">GHS {parseFloat(txn.product_total).toFixed(2)}</p>
