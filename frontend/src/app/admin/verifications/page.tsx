      ...
      <main className="mx-auto max-w-5xl px-4 py-4 sm:py-8">
        <div className="mb-6 sm:mb-8">
          <h1 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
            <ShieldCheck className="h-5 w-5 sm:h-6 sm:w-6 text-primary" /> Seller Verifications
          </h1>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                    ...
                    {v.status === 'PENDING' && (
                      <div className="flex gap-2 shrink-0 w-full sm:w-auto">
                        <Button size="sm" variant="outline" onClick={() => handleAction(v.id, 'reject')} className="rounded-full gap-1 border-red-200 text-red-700 hover:bg-red-50 flex-1 sm:flex-initial">
                          <XCircle className="h-3.5 w-3.5" /> Reject
                        </Button>
                        <Button size="sm" onClick={() => handleAction(v.id, 'approve')} className="rounded-full gap-1 flex-1 sm:flex-initial">
